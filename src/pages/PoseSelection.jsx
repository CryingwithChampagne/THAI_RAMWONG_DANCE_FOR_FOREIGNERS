...
const PoseCard = ({ pose, setSelectedPose, setCurrentPage, loadPoseDetails }) => {
  ...
  return (
    <div
      className={`bg-[#121826] border border-[#D4AF37]/20 hover:border-[#D4AF37] overflow-hidden transition-all duration-500 flex flex-col cursor-pointer group min-h-[360px] sm:h-[56vh] sm:max-h-[500px] ${isLoadingPose ? 'pointer-events-none opacity-75' : ''}`}
      onClick={async () => {
    ...
    >
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col items-center text-center justify-center relative z-30 -mt-8 sm:-mt-10">
        <h3 className="text-2xl sm:text-3xl font-serif mb-2 text-white group-hover:text-[#D4AF37] transition-colors drop-shadow-md">{pose.name}</h3>
        <p className="text-sm text-gray-400 leading-relaxed sm:px-4">{isLoadingPose ? 'Loading lesson assets...' : pose.desc}</p>
      </div>
    </div>
  );
};
...
const PoseSelection = ({ setCurrentPage, setSelectedPose }) => {
  ...
  return (
    <div className="w-full min-h-screen overflow-x-hidden bg-[#0B0F19] text-white flex flex-col">
      <div className="min-h-[65px] flex justify-between items-center gap-4 px-4 sm:px-8 md:px-16 py-3 border-b border-white/5 bg-[#0B0F19] z-40 relative shadow-md">
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <h2 className="text-base sm:text-xl font-serif text-[#D4AF37] uppercase tracking-wider sm:tracking-widest font-bold truncate">
            Select a Pose
          </h2>
        </div>
        <Button variant="outline" onClick={() => setCurrentPage('home')} className="text-xs px-4 sm:px-6 py-2.5 rounded-xl shrink-0">Exit</Button>
      </div>

      <div className="flex-1 flex items-start lg:items-center justify-center p-4 sm:p-6 lg:p-8 overflow-y-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 lg:gap-8 w-full max-w-7xl">
          {poses.map(pose => (
            <PoseCard
              key={pose.id}
              pose={pose}
